...
class ImpaintingImputation(BaseImputer):
    def __call__(self, img: torch.Tensor, mask: torch.Tensor):
        d_type = img.dtype
        ...
        dst = np.moveaxis(dst, -1, 0)
        dst = torch.from_numpy(dst)
        dst = dst.type(dtype=d_type)
        dst /= 255.0
        return  dst
    ...
    def batched_call(self, img: torch.Tensor, mask: torch.Tensor):
        ...
        for im, msk in zip(img, mask):
            ...
            ds = ds.type(dtype=d_type)
            ds /= 255.0
            sequece.append(ds)
            
        return torch.stack(sequece)
class ImpaintingImputationNS(BaseImputer):
    def __call__(self, img: torch.Tensor, mask: torch.Tensor):
        d_type = img.dtype
        img = np.moveaxis(img.numpy(), 0, -1)
        img = ((img - img.min()) * (1/(img.max() - img.min()) * 255)).astype('uint8')
